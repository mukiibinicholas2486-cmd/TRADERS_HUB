...
/* =======================================================
   WALK-FORWARD VALIDATION + NEXT-DIGIT EVIDENCE
======================================================= */

/*
 * Wilson lower bound for a binomial hit rate.
 * This is used as evidence strength, not as a guarantee.
 */
...
function validateDigitModel(
  ...
  return {
    ...
    recentAccuracy:
      recentWindow.length
        ? (recentWins / recentWindow.length) * 100
        : 0,

    averageActualProbability:
      probabilityHits.length
        ? mean(probabilityHits) * 100
        : 0
...
/*
 * Historical conditional test for the exact digit the live model
 * currently wants to use. This answers:
 * "When the model previously selected digit X, how often was the
 * very next digit actually X?"
 *
 * The current tick is never used to build the historical model.
 */
function validateCandidateDigit(
  digits,
  candidate,
  sampleCount
) {
...
function calculatePrediction() {
   ...
   * IMPORTANT:
   *
   * A live market cannot be proven to produce a particular next
   * digit. The strongest honest claim is historical, out-of-sample
   * evidence: when the same model selected this candidate before,
   * how often did the next observed digit match it?
   *
   * High confidence therefore requires:
   *   1) enough one-step-ahead validation samples;
   *   2) enough occurrences of the exact candidate;
   *   3) candidate hit rate above the 10% random baseline;
   *   4) the 95% Wilson lower bound still above baseline;
   *   5) recent candidate performance not collapsing.
   */
  const matchHigh =
    validation.samples >=
      state.bot.minimumSamples &&

    candidateEvidence.signals >=
      state.bot.minimumCandidateSignals &&

    candidateEvidence.accuracy >=
      state.bot.minimumCandidateAccuracy &&

    candidateEvidence.lower95 >
      state.bot.baselineDigitRate &&

    candidateEvidence.recentSignals >=
      state.bot.minimumRecentSignals &&

    candidateEvidence.recentAccuracy >=
      state.bot.minimumRecentAccuracy;
  ...
  return {
    ...
    match: {
      ...
      baselineAccuracy:
        validation.baselineAccuracy,

      lift:
        validation.lift,

      recentAccuracy:
        validation.recentAccuracy,
...
function renderPredictionPanel(
  ...
  if (!prediction) {
    ...
    container.innerHTML = `
      ...
      <div class="prediction-small">
        The scanner will show out-of-sample evidence for the next-digit candidate.
      </div>
  ...
  const evidence =
    prediction.match.candidateEvidence || {};
  ...
  const lower95 =
    Number(evidence.lower95 || 0).toFixed(1);

  const candidateAccuracy =
    Number(evidence.accuracy || 0).toFixed(1);

  const lift =
    Number(evidence.lift || 0).toFixed(1);

  const recentAccuracy =
    Number(evidence.recentAccuracy || 0).toFixed(1);
  ...
  container.innerHTML = `
    ...
    <div class="prediction-grid">
      ...
      <div class="prediction-box">
        ...
        <div class="prediction-small">
          Model score ${prediction.match.probability.toFixed(1)}%
        </div>

        <div class="prediction-small">
          Top candidates: ${topDigits}
        </div>
      ...
      <div class="prediction-box">
        ...
        <div class="prediction-small">
          ${evidence.wins || 0}/${evidence.signals || 0}
          times the model selected digit ${prediction.match.prediction}
        </div>

        <div class="prediction-small">
          95% lower bound: ${lower95}%
        </div>
      ...
      <div class="prediction-box">
        ...
        <div class="prediction-small">
          Random 10-digit baseline: 10.0%
        </div>

        <div class="prediction-small">
          Recent candidate accuracy: ${recentAccuracy}%
        </div>
    ...
    <div class="signal ${evidenceClass}" style="margin-top:10px;">
      ${evidenceTitle}
      • candidate ${prediction.match.prediction}
      • ${evidence.signals || 0} historical selections tested
    </div>

    <div class="prediction-small" style="margin-top:8px;">
      Evidence is out-of-sample historical testing. It does NOT prove the next live tick will be ${prediction.match.prediction}.
    </div>

    <div class="prediction-top">
      TOP DIGITS: ${topDigits}
    </div>
...
function updatePredictionBot() {
  ...
  log(
    `Scanner candidate ${prediction.match.prediction}: ${prediction.match.candidateEvidence.accuracy.toFixed(1)}% historical hit rate (${prediction.match.candidateEvidence.wins}/${prediction.match.candidateEvidence.signals}), 95% lower bound ${prediction.match.candidateEvidence.lower95.toFixed(1)}%.`
  );
  ...
  setText(
    "matchConf",
    prediction.match.highConfidence
      ? `VALIDATED • ${prediction.match.candidateEvidence.accuracy.toFixed(1)}% historical hit rate`
      : `NO PROOF • ${prediction.match.candidateEvidence.accuracy.toFixed(1)}% historical hit rate`
  );
  ...
  setText(
    "filterStatus",
    prediction.match.highConfidence
      ? `NEXT DIGIT ${prediction.match.prediction} • VALIDATED EVIDENCE`
      : highCount
        ...
        : "NO VALIDATED NEXT-DIGIT EVIDENCE"
  );
