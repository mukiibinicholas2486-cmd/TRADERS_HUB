...
=========================================================
 ...
 AUTO BOT IS OPTIONAL AND OFF BY DEFAULT.

 When enabled, it may place a trade only after the prediction confirmation gate opens a fresh proposal.

 This file does NOT guarantee profitable or timely trades.
=========================================================
*/
...
const state = {
  ...
  bot: {
     ...
     * A prediction is made from the completed tick.
     * The next tick must confirm that prediction before
     * a proposal can be requested or a contract bought.
     */
    pendingGate:
      null,
...
function resetMarketData() {
  ...
  if (state.bot.autoTrading) {
    disableAutoBot("AUTO BOT: MARKET RESET");
  }
...
function appendTick(
  ...
  /*
   * Confirm the prediction made on the previous tick
   * BEFORE calculating/arming the next prediction.
   */
  evaluatePredictionGate(digit);
...
function renderPredictionPanel(prediction) {
  const container = $("advancedPredictionDisplay");
  if (!container) return;
  const put = (id, value) => setText(id, value);
  if (!prediction) {
    container.innerHTML = `<div class="prediction-title">LIVE NEXT-DIGIT PREDICTION</div><div class="prediction-small">WAITING FOR VALIDATED TICK DATA</div><div class="prediction-wait">Ranked candidates, match probability and walk-forward validation will appear when enough ticks are available.</div>`;
    [
      ["predictionUpgradeStatus","WAITING FOR DATA"],["upgradeTopDigit","—"],["upgradeTopDigitMeta","No model candidate yet"],["upgradeMargin","—"],["upgradeValidation","—"],["upgradeValidationMeta","walk-forward samples"],["upgradeProbability","—"],["upgradeWins","—"],["upgradeSamples","—"],["upgradeMatchSignal","WAITING"],["upgradeMatchDetail","—"],["upgradeEvenSignal","WAITING"],["upgradeEvenDetail","—"],["upgradeOuSignal","WAITING"],["upgradeOuDetail","—"]
    ].forEach(([id,v])=>put(id,v));
    const b=$("upgradeDigitBars"), r=$("upgradeRecentDigits"); if(b)b.innerHTML=""; if(r)r.innerHTML=""; return;
  }
  const ranking = Array.isArray(prediction.match.ranking) ? prediction.match.ranking.slice(0,5) : [];
  const topDigits = ranking.map(item => `<span class="prediction-digit">${item.digit}<small>${(item.probability*100).toFixed(1)}%</small></span>`).join("");
  container.innerHTML = `<div class="prediction-title">LIVE NEXT-DIGIT PREDICTION</div><div class="prediction-small">${prediction.match.highConfidence ? "VALIDATED SIGNAL • CONSERVATIVE FILTER PASSED" : "MODEL CANDIDATE • NOT HIGH-CONFIDENCE"}</div><div class="prediction-grid"><div class="prediction-box"><div class="prediction-label">BEST MATCH</div><div class="prediction-value">${prediction.match.prediction}</div><div class="prediction-small">${prediction.match.probability.toFixed(1)}% model probability</div><div class="prediction-small">${prediction.match.accuracy.toFixed(1)}% walk-forward accuracy</div></div><div class="prediction-box"><div class="prediction-label">EVEN / ODD</div><div class="prediction-value small-value">${prediction.even.prediction}</div><div class="prediction-small">${prediction.even.probability.toFixed(1)}% model probability</div><div class="prediction-small">${prediction.even.accuracy.toFixed(1)}% validation</div></div><div class="prediction-box"><div class="prediction-label">OVER / UNDER</div><div class="prediction-value small-value">${prediction.over.prediction} ${prediction.over.threshold}</div><div class="prediction-small">${prediction.over.probability.toFixed(1)}% model probability</div><div class="prediction-small">${prediction.over.accuracy.toFixed(1)}% validation</div></div></div><div class="prediction-top">TOP DIGITS: ${topDigits}</div>`;
  put("predictionUpgradeStatus", prediction.match.highConfidence ? "HIGH-CONFIDENCE MODEL" : "LIVE CANDIDATE");
  put("upgradeTopDigit", String(prediction.match.prediction ?? "—"));
  put("upgradeTopDigitMeta", `${prediction.match.probability.toFixed(1)}% probability • #1 ranked candidate`);
  put("upgradeMargin", `${prediction.match.margin.toFixed(1)}%`);
  put("upgradeValidation", `${prediction.match.accuracy.toFixed(1)}%`);
  put("upgradeValidationMeta", `${prediction.match.samples} walk-forward samples`);
  put("upgradeProbability", `${prediction.match.probability.toFixed(1)}%`);
  put("upgradeWins", String(prediction.match.wins));
  put("upgradeSamples", String(prediction.match.samples));
  put("upgradeMatchSignal", `MATCH ${prediction.match.prediction}`);
  put("upgradeMatchDetail", `${prediction.match.probability.toFixed(1)}% probability • ${prediction.match.accuracy.toFixed(1)}% validation`);
  put("upgradeEvenSignal", prediction.even.prediction);
  put("upgradeEvenDetail", `${prediction.even.probability.toFixed(1)}% probability • ${prediction.even.accuracy.toFixed(1)}% validation`);
  put("upgradeOuSignal", `${prediction.over.prediction} ${prediction.over.threshold}`);
  put("upgradeOuDetail", `${prediction.over.probability.toFixed(1)}% probability • ${prediction.over.accuracy.toFixed(1)}% validation`);
  const bars=$("upgradeDigitBars"); if(bars) bars.innerHTML=ranking.map(item=>{const pct=Math.max(0,Math.min(100,item.probability*100));return `<div class="prediction-bar-row"><strong>${item.digit}</strong><div class="prediction-bar-track"><div class="prediction-bar-fill" style="width:${pct.toFixed(1)}%"></div></div><span class="prediction-bar-pct">${pct.toFixed(1)}%</span></div>`}).join("");
  const recent=$("upgradeRecentDigits"); if(recent){const digits=state.digits.slice(-16);recent.innerHTML=digits.map((d,i)=>`<span class="${i===digits.length-1?"latest":""}">${d}</span>`).join("");}
}


/* =======================================================
   NEXT-TICK PREDICTION CONFIRMATION GATE
======================================================= */

function selectedPredictionGate(prediction) {

  if (!prediction) return null;

  const contractType =
    String($("contractType")?.value || "DIGITMATCH").toUpperCase();

  if (!DIGIT_CONTRACT_TYPES.includes(contractType)) return null;

  if (contractType === "DIGITMATCH") {
    if (!Number.isInteger(prediction.match?.prediction)) return null;
    return {
      contractType,
      targetDigit: Number(prediction.match.prediction),
      predictedSide: `DIGIT ${prediction.match.prediction}`,
      predictionEpoch: state.lastMarketEpoch
    };
  }

  if (contractType === "DIGITEVEN" || contractType === "DIGITODD") {
    const even = prediction.even?.prediction === "EVEN";
    const predictedSide = even ? "EVEN" : "ODD";
    return {
      contractType,
      targetParity: predictedSide,
      predictedSide,
      predictionEpoch: state.lastMarketEpoch
    };
  }
  ...
  if (contractType === "DIGITOVER" || contractType === "DIGITUNDER") {
    const threshold = Number(prediction.over?.threshold);
    if (!Number.isInteger(threshold)) return null;
    const predictedSide = String(prediction.over?.prediction || "").toUpperCase();
    if (predictedSide !== "OVER" && predictedSide !== "UNDER") return null;
    return {
      contractType,
      threshold,
      predictedSide,
...
function predictionGateKey(gate) {
  if (!gate) return "";
  return [
    gate.contractType,
    gate.targetDigit ?? "",
    ...
    gate.predictedSide ?? "",
    gate.threshold ?? "",
    gate.confirmationEpoch ?? ""
  ].join("|");
}

function gateStatusText() {
  const confirmed = state.bot.confirmedGate;
  const pending = state.bot.pendingGate;
  const type = String($('contractType')?.value || 'DIGITMATCH').toUpperCase();

  if (confirmed && confirmed.contractType === type) {
    return `CONFIRMED • tick ${confirmed.confirmationDigit} matched ${confirmed.predictedSide} • PROPOSAL UNLOCKED`;
  }

  if (pending && pending.contractType === type) {
    return `PREDICTED ${pending.predictedSide} • WAITING FOR NEXT TICK TO CONFIRM`;
  }

  return "WAITING FOR PREDICTION";
}

function updatePredictionGateDisplay() {
  const text = gateStatusText();
  const el = $("predictionGateStatus");
  if (el) el.textContent = text;

  const tradeEl = $("tradePredictionStatus");
  if (tradeEl) tradeEl.textContent = text;
}

function resetPredictionGate() {
  state.bot.pendingGate = null;
  state.bot.confirmedGate = null;
  state.bot.proposalGateKey = null;
  updatePredictionGateDisplay();
}

function evaluatePredictionGate(actualDigit) {

  const gate = state.bot.pendingGate;

  if (!gate || !Number.isInteger(actualDigit)) return false;

  let confirmed = false;

  if (gate.contractType === "DIGITMATCH") {
    confirmed = actualDigit === gate.targetDigit;
  } else if (gate.contractType === "DIGITEVEN") {
    confirmed = actualDigit % 2 === 0;
  } else if (gate.contractType === "DIGITODD") {
    confirmed = actualDigit % 2 !== 0;
  } else if (gate.contractType === "DIGITOVER") {
    confirmed = actualDigit > gate.threshold;
  } else if (gate.contractType === "DIGITUNDER") {
    confirmed = actualDigit < gate.threshold;
  }
  ...
  state.bot.confirmedGate = {
    ...gate,
    confirmationDigit: actualDigit,
    confirmationEpoch: state.lastMarketEpoch
  };

  state.bot.proposalGateKey = predictionGateKey(state.bot.confirmedGate);
  updatePredictionGateDisplay();

  log(`Prediction CONFIRMED by next tick: ${actualDigit} matched ${gate.predictedSide}. Proposal unlocked.`);

  scheduleProposalRefresh(0);
  return true;
}
...
function updatePredictionBot() {
  ...
  /*
   * Arm the prediction for the NEXT tick.
   * It is not yet a trade signal.
   */
  state.bot.pendingGate =
    selectedPredictionGate(prediction);
  updatePredictionGateDisplay();
...
function invalidateTradingConnection() {
  ...
  if (state.bot.autoTrading) {
    disableAutoBot("AUTO BOT: TRADE CONNECTION LOST");
  }
...
function handleTradingMessage(
  ...
  if (
    ...
    const contracts =
      Array.isArray(data.portfolio.contracts)
    ...
    const open =
      contracts.find(contract => {
        ...
        return !["sold", "won", "lost", "expired"].includes(status);
      }) || null;

    if (open?.contract_id) {
      state.contractId = open.contract_id;
      setText("contractId", open.contract_id);
      setText("contractStatus", open.status || "OPEN");
      setText("contractProfit", open.profit ?? "—");
      setText("contractBuyPrice", open.buy_price ?? "—");
      ...
      try {
        sendTrading({
          ...
          contract_id: Number(open.contract_id),
          subscribe: 1,
          req_id: nextRequestId()
        });
      } catch (error) {
        log("Could not restore open contract stream: " + error.message);
      }
    } else {
...
function syncPredictionToTradePanel(
  ...
  /*
   * Never request a proposal from a prediction alone.
   * The next tick must first confirm the prediction.
   */
  if (!state.bot.confirmedGate) {
    return;
  }

  const params = getProposalParameters();
  const confirmed = state.bot.confirmedGate;

  if (params.contractType !== confirmed.contractType) {
    return;
  }

  if (
    state.bot.lastProposalPredictionKey ===
      state.bot.proposalGateKey &&
    state.proposal?.id
  ) {
    return;
  }
...
let proposalRefreshTimer = null;

function scheduleProposalRefresh(delay = 250) {

  if (proposalRefreshTimer) {
    clearTimeout(proposalRefreshTimer);
  }

  proposalRefreshTimer = setTimeout(() => {
    proposalRefreshTimer = null;
...
function updateProposal() {
  ...
  if (!confirmedGate) {
    setText("proposalId", "WAITING FOR NEXT-TICK CONFIRMATION");
    return;
  }

  if (confirmedGate.contractType !== params.contractType) {
    setText("proposalId", "WAITING FOR NEW CONFIRMATION");
    return;
  }

  const requestGateKey = predictionGateKey(confirmedGate);
  state.bot.proposalGateKey = requestGateKey;
  ...
  if (
    ...
    /* No barrier is sent for Even/Odd contracts. */
  }
  ...
  if (
    ...
    let barrier =
      String(confirmedGate.targetDigit);
    ...
    if (
      ...
      log(
        "DIGITMATCH proposal blocked: no digit barrier supplied."
      );
  ...
  if (
    ...
    let barrier =
      String(confirmedGate.threshold);
    ...
    if (
      ...
      log(
        "DIGITOVER proposal blocked: no threshold supplied."
      );
  ...
  if (
    ...
    if (
      ...
      log(
        "DIGITUNDER proposal blocked: no threshold supplied."
      );
...
function setAutoBotStatus(text) {
  setText("autoBotStatus", text);
  const btn = $("autoBotToggle");
  if (btn) {
    btn.textContent = state.bot.autoTrading ? "AUTO BOT: ON" : "AUTO BOT: OFF";
    btn.classList.toggle("active", state.bot.autoTrading);
  }
}

function canAutoBuy() {
  if (!state.bot.autoTrading) return false;
  if (state.bot.autoBuyInFlight) return false;
  if (!state.tradingReady || !state.account) return false;
  if (!state.bot.confirmedGate) return false;
  if (state.bot.proposalGateKey !== predictionGateKey(state.bot.confirmedGate)) return false;
  if (!state.proposal?.id) return false;
  if (Date.now() < state.bot.autoCooldownUntil) return false;
  if (state.bot.autoTradesThisSession >= state.bot.autoMaxTrades) return false;
  if (!canTrade()) return false;
  const age = state.lastLiveTickAt ? Date.now() - state.lastLiveTickAt : Infinity;
  if (age > 10000) return false;
  return true;
}

function enableAutoBot() {
  if (state.bot.autoTrading) return;

  const maxTrades = Number($("autoMaxTrades")?.value || 5);
  state.bot.autoMaxTrades = Math.max(1, Math.min(20, Number.isFinite(maxTrades) ? Math.floor(maxTrades) : 5));

  const ok = window.confirm(
    "Enable AUTO BOT?\n\n" +
    "The bot will automatically BUY only after the next-tick prediction confirmation gate succeeds and a fresh proposal is received.\n\n" +
    "It can place real trades on the selected account. Use a demo account first."
  );

  if (!ok) return;

  state.bot.autoTrading = true;
  state.bot.autoBuyInFlight = false;
  state.bot.autoTradesThisSession = 0;
  setAutoBotStatus("AUTO BOT: ON • WAITING FOR CONFIRMATION");
  log("AUTO BOT enabled. Waiting for a confirmed prediction and fresh proposal.");
  setTradingButtons(true);
}

function disableAutoBot(reason = "AUTO BOT: OFF") {
  state.bot.autoTrading = false;
  state.bot.autoBuyInFlight = false;
  setAutoBotStatus(reason);
  log("AUTO BOT disabled.");
  setTradingButtons(state.tradingReady);
}

function maybeAutoBuy() {
  if (!canAutoBuy()) {
    if (state.bot.autoTrading && !state.bot.confirmedGate) {
      setAutoBotStatus("AUTO BOT: ON • WAITING FOR CONFIRMATION");
    }
    return;
  }

  state.bot.autoBuyInFlight = true;
  state.bot.autoCooldownUntil = Date.now() + state.bot.autoCooldownMs;
  setAutoBotStatus("AUTO BOT: EXECUTING CONFIRMED TRADE…");
  log("AUTO BOT: confirmed gate + fresh proposal received. Executing BUY.");

  buyContract({ auto: true });
}
...
function buyContract(options = {}) {
  ...
  if (
    ...
    log("BUY blocked: waiting for the predicted condition to be confirmed by the next market tick.");
    return;
  }
  ...
  try {
    ...
    log(
      isAuto
        ? "AUTO BOT: Buy request sent at confirmed proposal."
        : "Buy request sent."
    );

    if (isAuto) {
      state.bot.autoTradesThisSession += 1;
      state.bot.autoBuyInFlight = false;
      state.bot.autoCooldownUntil = Date.now() + state.bot.autoCooldownMs;
      state.proposal = null;
      state.bot.confirmedGate = null;
      state.bot.proposalGateKey = null;
      setText("proposalId", "TRADE SENT");
      setText("askPrice", "—");
      setText("payout", "—");

      if (state.bot.autoTradesThisSession >= state.bot.autoMaxTrades) {
        disableAutoBot("AUTO BOT: SESSION LIMIT REACHED");
      } else {
        setAutoBotStatus(`AUTO BOT: ON • ${state.bot.autoTradesThisSession}/${state.bot.autoMaxTrades} trades`);
      }
    }
    ...
    if (isAuto) {
      state.bot.autoBuyInFlight = false;
      setAutoBotStatus("AUTO BOT: ON • BUY FAILED — WAITING");
    }
...
function invalidateProposal(resetGate = true) {
...
function updatePositionPanel() {
  ...
  const empty = $("positionEmpty");
  const position = $("position");

  if (!state.account) {
    if (empty) {
      empty.textContent = "Log in and select an account.";
      empty.style.display = "";
    }
    if (position) {
  ...
  const hasPosition = Boolean(state.contractId);

  if (empty) {
    empty.textContent = hasPosition
      ? ""
      : "No open position.";
    empty.style.display = hasPosition ? "none" : "";
  }

  if (position) {
    position.style.display = hasPosition ? "" : "none";
  }
}
...
function refreshOpenPosition() {
  ...
  try {
    ...
    log("Open position refreshed.");
  } catch (error) {
    log("Could not refresh open position: " + error.message);
  }
}
...
function setupAccountControls() {
  ...
  if (refreshBtn) {
    refreshBtn.onclick = refreshOpenPosition;
  }
